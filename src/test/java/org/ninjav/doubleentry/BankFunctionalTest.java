 ...
 *
 * <b>Important!</b> It is not allowed to modify this test class in any way.
 */
public class BankFunctionalTest {
    // There are no semantics in these constants, its just names

    private static final String CASH_ACCOUNT_1 = "cash_1_ZAR";

    private static final String CASH_ACCOUNT_2 = "cash_2_SEK";

    private static final String REVENUE_ACCOUNT_1 = "revenue_1_ZAR";
    ...
    public void accountBalancesUpdatedAfterTransfer() {
        accountService.createAccount(CASH_ACCOUNT_1, toMoney("1000.00", "ZAR"));
        accountService.createAccount(REVENUE_ACCOUNT_1, toMoney("0.00", "ZAR"));

        Assert.assertEquals(toMoney("1000.00", "ZAR"), accountService.getAccountBalance(CASH_ACCOUNT_1));
        Assert.assertEquals(toMoney("0.00", "ZAR"), accountService.getAccountBalance(REVENUE_ACCOUNT_1));

        transferService.transferFunds(TransferRequest.builder()
                .reference("T1").type("testing")
                .account(CASH_ACCOUNT_1).amount(toMoney("-5.00", "ZAR"))
                .account(REVENUE_ACCOUNT_1).amount(toMoney("5.00", "ZAR"))
                .build());

        transferService.transferFunds(TransferRequest.builder()
                .reference("T2").type("testing")
                .account(CASH_ACCOUNT_1).amount(toMoney("-10.50", "ZAR"))
                .account(REVENUE_ACCOUNT_1).amount(toMoney("10.50", "ZAR"))
                .build());

        Assert.assertEquals(toMoney("984.50", "ZAR"), accountService.getAccountBalance(CASH_ACCOUNT_1));
        Assert.assertEquals(toMoney("15.50", "ZAR"), accountService.getAccountBalance(REVENUE_ACCOUNT_1));
    ...
    public void accountBalancesUpdatedAfterMultileggedTransfer() {
        ...
        transferService.transferFunds(TransferRequest.builder()
                ...
                .account(CASH_ACCOUNT_1).amount(toMoney("-10.50", "ZAR"))
                .account(REVENUE_ACCOUNT_1).amount(toMoney("10.50", "ZAR"))
                .account(CASH_ACCOUNT_1).amount(toMoney("-2.00", "ZAR"))
                .account(REVENUE_ACCOUNT_1).amount(toMoney("1.00", "ZAR"))
                .account(REVENUE_ACCOUNT_1).amount(toMoney("1.00", "ZAR"))
                .build());

        Assert.assertEquals(toMoney("982.50", "ZAR"), accountService.getAccountBalance(CASH_ACCOUNT_1));
        Assert.assertEquals(toMoney("17.50", "ZAR"), accountService.getAccountBalance(REVENUE_ACCOUNT_1));
    }
    ...
    public void accountBalancesUpdatedAfterMultileggedMulticurrencyTransfer() {
        ...
        Assert.assertEquals(toMoney("995.00", "ZAR"), accountService.getAccountBalance(CASH_ACCOUNT_1));
        Assert.assertEquals(toMoney("5.00", "ZAR"), accountService.getAccountBalance(REVENUE_ACCOUNT_1));
